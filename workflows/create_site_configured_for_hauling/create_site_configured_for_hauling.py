...
arg_parser.add_argument("--log_level", default=logging.DEBUG)
...
material_waste_id = create_material(a_site_id=site_id, a_server_config=server, a_material_name="Waste", a_headers=header_json)
...
material_overburden_id = create_material(a_site_id=site_id, a_server_config=server, a_material_name="Overburden", a_headers=header_json, a_accepted_measurements=[accepted_measurement_volume,accepted_measurement_weight], a_default_state=state_default, a_additional_states=[state_excavated], a_haul_mixin = haul_mixin)
...
create_region(a_region_name="Discovery", a_site_id=site_id, a_server_config=server, a_verticies_file=args.region_discovery_verticies_file, a_headers=header_json, a_discoverable=True)
create_region(a_region_name="Load Waste", a_site_id=site_id, a_server_config=server, a_verticies_file=args.region_load_verticies_file, a_headers=header_json, a_discoverable=False, a_color="#088a08", a_coordinate_system="wgs84", a_opacity=50, a_haul_mixin=load_waste_region_haul_mixin)
create_region(a_region_name="Dump Waste", a_site_id=site_id, a_server_config=server, a_verticies_file=args.region_dump_verticies_file, a_headers=header_json, a_discoverable=False, a_color="#ff7f00", a_coordinate_system="wgs84", a_opacity=50, a_haul_mixin=dump_waste_region_haul_mixin)
...
for idx,item in enumerate(delays):
    create_delay(a_site_id=site_id, a_server_config=server, a_delay_name=item, a_delay_code=str(idx), a_headers=header_json)


###### Create a descriptively named Task for Hauler operators to select on their apps
logging.info("creating task")
rj = create_task(a_server_config=server, a_site_id=site_id, a_task_name="Haul Waste", a_headers=header_json)
...
create_auth_code(a_server_config=server, a_site_id=site_id, a_code_name="Hauling Code", a_code_pin=args.site_auth_code, a_headers=header_json)